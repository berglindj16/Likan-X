...
timar = []
dags="""select distinct (dagsetning) from receipt where dagsetning > '2018-12-05' and dagsetning < '2018-12-09'
...
heild1 = 0
heild2 = 0
heild_hilluplass = """select sum(magn) from hilluplass, voruspjald where itemcode = '05' and hilluplass.itemno = voruspjald.itemno and voruspjald.millilitrar = '{}';"""


listi_ml = """select distinct(voruspjald.millilitrar) from voruspjald, hilluplass where itemcode = '05' and hilluplass.itemno = voruspjald.itemno group by millilitrar order by voruspjald.millilitrar"""
...
sell_items ="""select sum(-1*i.quantity),r.dagsetning from receipt r, receiptitems i, voruspjald v where r.receiptno = i.receiptno and i.itemno = v.itemno and i.itemno = '{}' and r.timasetning > '{}' and r.timasetning < '{}' and r.dagsetning = '{}' and v.itemcode = '05' group by r.dagsetning, r.timasetning order by r.dagsetning, r.timasetning"""
litrar = """select millilitrar from voruspjald where itemno = '{}';"""


C = 100; 
MaxSlots = int(heild[0][0]/8)+int(heild1/10)+int(heild2/5);
MinSlots = 1;
M = 10000; #Stora M
