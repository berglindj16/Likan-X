...
items = "select hilluplass.itemno from voruspjald,hilluplass where itemcode ='16' and hilluplass.itemno = voruspjald.itemno order by hilluplass.itemno"
cursor.execute(items)
result = cursor.fetchall()

heild1 = 0
heild2 = 0
heild_hilluplass = """select sum(magn) from hilluplass, voruspjald where itemcode = '16' and hilluplass.itemno = voruspjald.itemno and voruspjald.millilitrar = '{}';"""


listi_ml = """select distinct(voruspjald.millilitrar) from voruspjald, hilluplass where itemcode = '16' and hilluplass.itemno = voruspjald.itemno group by millilitrar order by voruspjald.millilitrar"""
...
sell_items ="""select sum(-1*i.quantity),r.dagsetning from receipt r, receiptitems i, voruspjald v where r.receiptno = i.receiptno and i.itemno = v.itemno and i.itemno = '{}' and r.timasetning > '{}' and r.timasetning < '{}' and r.dagsetning = '{}' and v.itemcode = '16' group by r.dagsetning, r.timasetning order by r.dagsetning, r.timasetning"""
litrar = """select millilitrar from voruspjald where itemno = '{}';"""


C = 100; 
MaxSlots = int(heild1/12)+int(heild2/7);
MinSlots = 1;
M = 10000; 
