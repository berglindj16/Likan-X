...
items = """select hilluplass.itemno from voruspjald, hilluplass where itemcode in ('60', '61', '62') and magn <= '96' and hilluplass.itemno = voruspjald.itemno order by hilluplass.magn desc;"""
cursor.execute(items)
result = cursor.fetchall()

heild1 = 0
heild_hilluplass = """select sum(magn) from hilluplass, voruspjald where itemcode in ('60', '61', '62') and magn <= '96' and hilluplass.itemno = voruspjald.itemno and voruspjald.millilitrar = '{}';"""


listi_ml = """select distinct(voruspjald.millilitrar) from voruspjald, hilluplass where itemcode in ('60', '61', '62') and magn <= '96' and hilluplass.itemno = voruspjald.itemno group by millilitrar order by voruspjald.millilitrar"""
...
for i in range(len(flaska)):
	cursor.execute(heild_hilluplass.format(flaska[i]))
